class Visualizer {
    static drawNetwork(ctx, network){
        ...
        const levelsQuantity = network.levels.length;

        for(let i = levelsQuantity-1 ; i >= 0; --i) {
            const levelTop = top + 
                lerp(height-levelHeight, 0, 
                    levelsQuantity == 1 ? 0.5 : i/(levelsQuantity-1));

            ctx.setLineDash([7,3]);
            Visualizer.drawLevel(ctx, network.levels[i], left, levelTop, width, levelHeight, 
                i == levelsQuantity-1 ? ['up', 'left', 'right', 'down'] : []);
        }
    }
